function SetRow({ set, setIndex, onUpdate, onRemove, showRemove, showNotes }) {
  return (
    <div className="group grid grid-cols-12 gap-2 rounded-xl border border-app-subtle bg-surface px-2 py-2 transition hover:border-blue-500/40">
      <div className="col-span-3 flex items-center text-sm text-app-muted">Set {set.setNumber}</div>
      <input
        type="number"
        min="1"
      ...
      <div className={`col-span-12 overflow-hidden transition-all duration-200 ${showNotes ? 'max-h-20' : 'max-h-0'}`}>
        <input
          type="text"
          value={set.notes}
